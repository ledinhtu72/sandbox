...
async function dbLoad() {
  ...
  for (const user of userModels) {
    // Create using the schema field names (first_name / last_name)
    ...
    try {
      ...
      // Attach mapping back to the fake model so comments can reference
  ...
  try {
    const schemaInfoObj = await SchemaInfo.create({
      version: versionString,
    });
    console.log(
      "SchemaInfo object created with version ",
      schemaInfoObj.version
    );
  } catch (error) {
